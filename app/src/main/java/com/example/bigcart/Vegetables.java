...
public class Vegetables extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vegrecv=findViewById(R.id.vegrecv);
    ...
    private void getVegetables() {
        ...
        supabaseClient.fetchVegetables(new SupabaseClient.SBC_Callback() {
            @Override
            public void onFailure(IOException e) {
                runOnUiThread(() -> {
                    Log.e("getVegetables:onFailure", e.getLocalizedMessage());
                });
            }
            ...
            public void onResponse(String responseBody) {
                runOnUiThread(() -> {
                    Log.e("getVegetables:onResponse", responseBody);
                    Gson gson = new Gson();
                    Type type = new TypeToken<List<com.example.bigcart.Models.Product>>() {
                    }.getType();
                    List<Product> productList = gson.fromJson(responseBody, type);
                    VegetablesAdapter vegetablesAdapter = new VegetablesAdapter(getApplicationContext(), productList);
                    vegrecv.setAdapter(vegetablesAdapter);
                    vegrecv.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
                });
            }
        });
    }
